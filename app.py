...
# Define BMI calculation function at the beginning
...
height = st.sidebar.slider("Height (cm)", 150, 200, 175)
weight = st.sidebar.slider("Weight (kg)", 45, 120, 70)
chest = st.sidebar.slider("Chest (cm)", 75, 135, 100)
waist = st.sidebar.slider("Waist (cm)", 60, 110, 80)
hip = st.sidebar.slider("Hip (cm)", 80, 130, 95)
gender = st.sidebar.selectbox("Gender", ['Male', 'Female', 'Non-Binary'])
age = st.sidebar.slider("Age", 18, 70, 35)
brand = st.sidebar.selectbox("Brand", ['Zara', 'H&M', 'Nike', 'Levi\'s', 'Uniqlo'])
fit_feedback = st.sidebar.selectbox("Fit Feedback", ['Too Small', 'Fit Well', 'Too Large'])
return_exchange_status = st.sidebar.selectbox("Return/Exchange Status", ['Kept', 'Returned', 'Exchanged'])
loyalty_status = st.sidebar.selectbox("Loyalty Status", ['Frequent Buyer', 'Occasional Buyer', 'New Buyer'])

# Collect base_size from the user
base_size = st.sidebar.selectbox("Base Size", ['XS', 'S', 'M', 'L', 'XL', 'XXL'])
...
user_features = torch.tensor([[height, weight, chest, waist, hip, calculate_bmi(height, weight)] + [1 if base_size == size else 0 for size in ['XS', 'S', 'M', 'L', 'XL', 'XXL']] + [apply_weights(df, height, weight, chest, waist, hip, gender, age, brand, fit_feedback, return_exchange_status, loyalty_status)]], dtype=torch.float32)
...
# Display the cluster and recommended size chart
...
# Display the recommended size chart based on cluster
if cluster_assignments in size_charts:
    st.write("### Recommended Size Chart")
    st.write(pd.DataFrame([size_charts[cluster_assignments]]))
else:
